...
def plot_adsorbed_circles(adsorbed_x, adsorbed_y, radius, width, breadth, reference_indices=[]):
    import numpy as np
    ...
    plt.axis([-0.1*breadth, breadth*1.1, -0.1*width, width*1.1])
...
def readFile(name):
    import numpy as np
    fileInput = open('ParticleData/data_' + name, 'r')
    x = []
    y = []
    a = []
